...
$translations = [
    'en' => [
        ...
        // Lecture 3
        'lecture3-title' => 'Lecture 3: Memory Hierarchy & Virtual Memory',
        'welcome-text-lecture3' => 'Select a scenario to explore how memory hierarchies and virtual memory behave step-by-step.',
        'lecture3-topology-heading' => 'System Topology',
        'lecture3-timeline-heading' => 'Step Timeline',
        'lecture3-step-indicator' => 'Step {current} of {total}',
        'lecture3-component-cpu' => 'CPU Core',
        'lecture3-component-l1' => 'L1 Cache',
        'lecture3-component-l2' => 'L2 Cache',
        'lecture3-component-memory' => 'Main Memory (DRAM)',
        'lecture3-component-disk' => 'Storage',
        'lecture3-component-page-table' => 'Page Table',
        'lecture3-component-tlb' => 'TLB (Translation Lookaside Buffer)',
        'lecture3-component-os' => 'OS Kernel',
        'scenario-memory-hierarchy' => 'Scenario 0: Memory Hierarchy Overview',
        'scenario-cache-hit' => 'Scenario 1: Cache Hit (Direct-Mapped)',
        'scenario-cache-miss' => 'Scenario 2: Cache Miss & Replacement',
        'scenario-virtual-memory' => 'Scenario 3: Virtual Memory Page Fault',
        'memory-hierarchy-description' => 'Follow how a request flows from the CPU through the cache hierarchy and back.',
        'cache-hit-description' => 'Watch a direct-mapped L1 cache satisfy an instruction fetch with minimal latency.',
        'cache-miss-description' => 'Inspect what happens during a miss that requires refilling a cache line from DRAM.',
        'virtual-memory-description' => 'Trace the sequence of events when a virtual address triggers a page fault.',
        'memory-hierarchy-step-1-title' => 'CPU issues a request',
        'memory-hierarchy-step-1-text' => 'The core needs word 0x1A2C for the current instruction. The load pipeline checks the closest L1 cache first.',
        'memory-hierarchy-step-2-title' => 'L1 miss escalates to L2',
        'memory-hierarchy-step-2-text' => 'Line 12 holds an unrelated tag, so the request walks to the larger L2 cache that serves the entire core.',
        'memory-hierarchy-step-3-title' => 'L2 finds the block',
        'memory-hierarchy-step-3-text' => 'The matching tag is present in L2. It streams the aligned 64-byte block from its data array toward the core.',
        'memory-hierarchy-step-4-title' => 'Block fills L1 and execution resumes',
        'memory-hierarchy-step-4-text' => 'The block installs into the now-vacant L1 line. The CPU consumes the requested word and continues execution.',
        'cache-hit-step-1-title' => 'Address decode for fetch',
        'cache-hit-step-1-text' => 'An instruction fetch targets address 0x3F20. The index bits select line 7 inside the direct-mapped L1 I-cache.',
        'cache-hit-step-2-title' => 'Tag matches cached line',
        'cache-hit-step-2-text' => 'The stored tag equals 0x3F, so the requested bytes are already resident. This is a warm cache line.',
        'cache-hit-step-3-title' => 'Serve data from L1',
        'cache-hit-step-3-text' => 'The bytes flow from L1 directly into the instruction decoder. No other hierarchy level is touched.',
        'cache-hit-step-4-title' => 'Counters record the hit',
        'cache-hit-step-4-text' => 'Hardware increments the hit counter. The operating system can sample these metrics to tune workloads.',
        'cache-miss-step-1-title' => 'Cold miss detected',
        'cache-miss-step-1-text' => 'A data load to 0x8AC4 maps to line 2 in L1. The tag comparison fails, signalling a miss.',
        'cache-miss-step-2-title' => 'Probe deeper levels',
        'cache-miss-step-2-text' => 'L2 is consulted next but also misses. The memory controller issues a read for the aligned block at 0x8A80.',
        'cache-miss-step-3-title' => 'Victim write-back and refill',
        'cache-miss-step-3-text' => 'The old cache line was dirty, so it is written back while DRAM returns the new block toward the core.',
        'cache-miss-step-4-title' => 'Hierarchy updated',
        'cache-miss-step-4-text' => 'Fresh data fills both L1 and L2. The CPU retries the load and receives the requested word.',
        'virtual-memory-step-1-title' => 'Virtual address generated',
        'virtual-memory-step-1-text' => 'The CPU issues virtual address 0x7F45. The translation lookaside buffer (TLB) is checked for a cached mapping.',
        'virtual-memory-step-2-title' => 'TLB miss raises a fault',
        'virtual-memory-step-2-text' => 'No valid translation exists. The hardware raises a page fault so the OS can provide page 0x7F.',
        'virtual-memory-step-3-title' => 'Operating system brings the page in',
        'virtual-memory-step-3-text' => 'The OS selects a free frame, asks the storage device for page 0x7F, and waits for the DMA transfer to finish.',
        'virtual-memory-step-4-title' => 'Tables updated and execution resumes',
        'virtual-memory-step-4-text' => 'The page table and TLB receive the new mapping. The faulting instruction restarts with a valid translation.',
        'lecture3-status-requesting' => 'Requesting address {address}',
        'lecture3-status-l1-check' => 'Checking tag for line {line}',
        'lecture3-status-hit' => 'Cache hit! Data is ready.',
        'lecture3-status-miss' => 'Cache miss — escalating to the next level.',
        'lecture3-status-fetching' => 'Fetching block starting at {address}',
        'lecture3-status-delivering' => 'Delivering data to the core',
        'lecture3-status-ready' => 'Ready for the next instruction',
        'lecture3-status-page-fault' => 'Page fault on page {page}',
        'lecture3-status-page-loaded' => 'Page {page} now resides in frame {frame}',
        'lecture3-status-os-handling' => 'OS trap handler is orchestrating the transfer',
        'lecture3-status-tlb-update' => 'TLB entry refreshed with the new mapping',
        'lecture3-status-l2-update' => 'Updating shared cache copy',
        'lecture3-status-disk-fetch' => 'Disk is streaming page {page}',
        'lecture3-status-memory-allocate' => 'Allocating physical frame {frame}',
        'lecture3-status-os-metrics' => 'Collecting cache statistics for profiling',
        'lecture3-status-writeback' => 'Writing back victim data before refill',
        'lecture3-status-executing' => 'Executing the fetched instruction',
        'lecture3-status-memory-transfer' => 'Streaming the block toward the caches',
    ...
    'ru' => [
        ...
        // Лекция 3
        'lecture3-title' => 'Лекция 3: Иерархия памяти и виртуальная память',
        'welcome-text-lecture3' => 'Выберите сценарий, чтобы пошагово изучить работу иерархии памяти и виртуальной памяти.',
        'lecture3-topology-heading' => 'Топология системы',
        'lecture3-timeline-heading' => 'Хронология шагов',
        'lecture3-step-indicator' => 'Шаг {current} из {total}',
        'lecture3-component-cpu' => 'Ядро CPU',
        'lecture3-component-l1' => 'Кэш L1',
        'lecture3-component-l2' => 'Кэш L2',
        'lecture3-component-memory' => 'ОЗУ (DRAM)',
        'lecture3-component-disk' => 'Накопитель',
        'lecture3-component-page-table' => 'Таблица страниц',
        'lecture3-component-tlb' => 'TLB (буфер трансляций)',
        'lecture3-component-os' => 'Ядро ОС',
        'scenario-memory-hierarchy' => 'Сценарий 0: Обзор иерархии памяти',
        'scenario-cache-hit' => 'Сценарий 1: Попадание в кэш (прямое отображение)',
        'scenario-cache-miss' => 'Сценарий 2: Промах кэша и замещение',
        'scenario-virtual-memory' => 'Сценарий 3: Ошибка страницы виртуальной памяти',
        'memory-hierarchy-description' => 'Проследите, как запрос проходит от процессора через уровни кэша и возвращается обратно.',
        'cache-hit-description' => 'Посмотрите, как кэш L1 с прямым отображением обслуживает выборку инструкции с минимальной задержкой.',
        'cache-miss-description' => 'Разберите события при промахе, когда строку кэша нужно загрузить из ОЗУ.',
        'virtual-memory-description' => 'Отследите последовательность событий при обращении к виртуальному адресу, вызывающему ошибку страницы.',
        'memory-hierarchy-step-1-title' => 'Процессор формирует запрос',
        'memory-hierarchy-step-1-text' => 'Ядру требуется слово 0x1A2C для текущей инструкции. Конвейер загрузки сначала проверяет ближайший кэш L1.',
        'memory-hierarchy-step-2-title' => 'Промах L1 передаёт запрос в L2',
        'memory-hierarchy-step-2-text' => 'В строке 12 хранится другой тег, поэтому запрос направляется в более крупный кэш L2, обслуживающий всё ядро.',
        'memory-hierarchy-step-3-title' => 'L2 находит нужный блок',
        'memory-hierarchy-step-3-text' => 'В L2 есть совпадающий тег. Он передаёт выровненный 64-байтный блок из своей матрицы данных к ядру.',
        'memory-hierarchy-step-4-title' => 'Блок заполняет L1, выполнение продолжается',
        'memory-hierarchy-step-4-text' => 'Блок устанавливается в освободившуюся строку L1. Процессор получает требуемое слово и продолжает выполнение.',
        'cache-hit-step-1-title' => 'Декодирование адреса для выборки',
        'cache-hit-step-1-text' => 'Выборка инструкции обращается к адресу 0x3F20. Биты индекса выбирают строку 7 в прямом кэше инструкций L1.',
        'cache-hit-step-2-title' => 'Тег совпадает с кэшированной строкой',
        'cache-hit-step-2-text' => 'Хранимый тег равен 0x3F, поэтому нужные байты уже находятся в кэше. Это «тёплая» строка.',
        'cache-hit-step-3-title' => 'Обслуживание напрямую из L1',
        'cache-hit-step-3-text' => 'Байты поступают из L1 прямо в декодер инструкций. Более низкие уровни иерархии не задействуются.',
        'cache-hit-step-4-title' => 'Счётчики фиксируют попадание',
        'cache-hit-step-4-text' => 'Аппаратное обеспечение увеличивает счётчик попаданий. ОС может считывать эти метрики для оптимизации рабочих нагрузок.',
        'cache-miss-step-1-title' => 'Обнаружен холодный промах',
        'cache-miss-step-1-text' => 'Загрузка данных по адресу 0x8AC4 отображается в строку 2 L1. Сравнение тегов неудачно — фиксируется промах.',
        'cache-miss-step-2-title' => 'Проверка более глубоких уровней',
        'cache-miss-step-2-text' => 'Затем опрашивается L2, но тоже безуспешно. Контроллер памяти запрашивает выровненный блок по адресу 0x8A80.',
        'cache-miss-step-3-title' => 'Запись вытесняемой строки и подкачка',
        'cache-miss-step-3-text' => 'Старая строка была «грязной», поэтому она записывается обратно, пока DRAM возвращает новый блок к ядру.',
        'cache-miss-step-4-title' => 'Иерархия обновляется',
        'cache-miss-step-4-text' => 'Новые данные заполняют кэши L1 и L2. Процессор повторяет загрузку и получает нужное слово.',
        'virtual-memory-step-1-title' => 'Сформирован виртуальный адрес',
        'virtual-memory-step-1-text' => 'Процессор выдаёт виртуальный адрес 0x7F45. Буфер трансляций (TLB) проверяется на наличие готового отображения.',
        'virtual-memory-step-2-title' => 'Промах TLB вызывает исключение',
        'virtual-memory-step-2-text' => 'Действительного отображения нет. Аппаратно генерируется исключение «ошибка страницы», чтобы ОС загрузила страницу 0x7F.',
        'virtual-memory-step-3-title' => 'Операционная система подкачивает страницу',
        'virtual-memory-step-3-text' => 'ОС выбирает свободный фрейм, запрашивает у накопителя страницу 0x7F и ожидает завершения DMA-передачи.',
        'virtual-memory-step-4-title' => 'Таблицы обновлены, выполнение возобновляется',
        'virtual-memory-step-4-text' => 'Таблица страниц и TLB получают новое отображение. Прерванная инструкция запускается повторно уже с корректной трансляцией.',
        'lecture3-status-requesting' => 'Запрашивается адрес {address}',
        'lecture3-status-l1-check' => 'Проверка тега для строки {line}',
        'lecture3-status-hit' => 'Попадание в кэш — данные готовы.',
        'lecture3-status-miss' => 'Промах кэша — переход на следующий уровень.',
        'lecture3-status-fetching' => 'Выборка блока с адреса {address}',
        'lecture3-status-delivering' => 'Данные передаются к ядру',
        'lecture3-status-ready' => 'Готово к следующей инструкции',
        'lecture3-status-page-fault' => 'Ошибка страницы для страницы {page}',
        'lecture3-status-page-loaded' => 'Страница {page} размещена во фрейме {frame}',
        'lecture3-status-os-handling' => 'Обработчик ОС управляет передачей',
        'lecture3-status-tlb-update' => 'Запись TLB обновлена новым отображением',
        'lecture3-status-l2-update' => 'Обновление общей копии блока',
        'lecture3-status-disk-fetch' => 'Накопитель подаёт страницу {page}',
        'lecture3-status-memory-allocate' => 'Выделяется физический фрейм {frame}',
        'lecture3-status-os-metrics' => 'Сбор статистики кэша для профилирования',
        'lecture3-status-writeback' => 'Запись вытесняемой строки перед перезаполнением',
        'lecture3-status-executing' => 'Выполняется полученная инструкция',
        'lecture3-status-memory-transfer' => 'Поток блока направляется к кэшам',
